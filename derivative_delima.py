...
class scene1(Scene):
    def construct(self):
        ...
        """Scene 1"""
        title=Text("The Derivative Delima", gradient=(RED, BLUE, GREEN), font_size=50)

        subtitle=MathTex(r"\frac{d}{dx}f(x)=f’(x)", font_size=30, substrings_to_isolate=["x"])
        ...
        """Scene 2"""
        eqns1_s2 = MathTex(r"a \times b = a + a + a + \ldots + a ~~~~  (b~ \text{times})", substrings_to_isolate=["a", "b"])
        eqns1_s2.set_color_by_tex_to_color_map({"a": YELLOW,
                                                "b": RED})
        self.play(FadeIn(eqns1_s2))
        self.play(ApplyMethod(eqns1_s2.shift,2*UP))
        self.wait(2)
        ...
        eqns2_s4 = MathTex(r" x^2 = x +x + x \ldots~ (x~ \mathrm{times})", substrings_to_isolate=["x"])
        ...
        eqns1_s4 = MathTex(r"\frac{d}{dx}x^2 = \frac{d}{dx}(x +x + x + \ldots) \quad (x~ \mathrm{times})", substrings_to_isolate=["x"])
        eqns1_s4.set_color_by_tex_to_color_map({"x": ORANGE})
        self.play(ApplyMethod(eqns1_s4.shift,2*UP))
        self.wait(2)

        eqns2_s4 = MathTex(r"{{2x= \frac{d}{dx}x + \frac{d}{dx}x + \frac{d}{dx}x + \ldots \quad (x}}~\mathrm{times})", substrings_to_isolate=["x"])
